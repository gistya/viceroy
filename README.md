...
But since we can't... Viceroy is here to provide a 🛡️ and a modern alternative to those >25-year-old C-based options with all their subdependencies. My final straw was trying to just build a Swift library on Windows. It was supposed to have no dependencies, but somehow, iconv got smuggled in with the bathwater. Then, even after doing all of the raindances demanded by the CMAKE ball and chain, the build broke because of an arcane `iconv` vs. `libiconv` naming difference. 
